...
if (options.httpCookies) phantom.addCookie(options.httpCookies)
...
function waitFor(testFx, onReady) {
  var condition = false
    interval = setInterval(function() {
      if (!condition ) {
        condition = requestsArray.length === 0 && (typeof(testFx) === "string" ? eval(testFx) : testFx());
      } else {
...
page.onLoadFinished = function (status) {
  ...
  if (options.waitFor) {
    waitFor("page.evaluate(function(){"+options.waitFor+"})", onReady);
  } else {
    onReady();
  }
}
